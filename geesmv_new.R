GEE.var.md.new<-function (formula, id, family = gaussian, data, corstr = "independence") 
{
  require(matrixcalc)
  if (is.null(data$id)) {
...
GEE.var.kc.new<-function (formula, id, family = gaussian, data, corstr = "independence") 
{
  require(matrixcalc)
  if (is.null(data$id)) {
    index <- which(names(data) == id)
