...
for (ind in 1:2){
  ...
  filename <- paste("conResults/final_con", namei, ".xlsx", sep="")
  data <- read.xlsx(filename, colNames=TRUE)
  colnames(data)[17:21]<-c("MB.1","ROB.1","KC.1","MD.1","AVG.1")
  assign(paste("final_con", namei, sep=""), data)
}
...
# MAEE
data_plot <- final_con_MAEE[, -(10:16)]
data_plot <- data_plot[, -14]
data_plot <- data_plot[-(31:36), ]
colnames(data_plot)[10:13] <- c("MB", "ROB", "KC", "MD")
...
data_plot <- melt(data_plot, id.vars = c("b", "alpha0", "alpha1", "alpha2", "N", "I", "K", "t", "phi", "model", "scenario"),
                  measure.vars = c("MB", "ROB", "KC", "MD"),
                  variable.name = "var", value.name = "size")
min(data_plot$size) # 0.036
max(data_plot$size) # 0.092
...
data_plot <- melt(data_plot, id.vars = c("b", "alpha0", "alpha1", "alpha2", "N", "I", "K", "t", "phi", "model", "scenario"),
                  measure.vars = c("MB", "ROB", "KC", "MD"),
                  variable.name = "var", value.name = "size")
min(data_plot$size) # 0.004
max(data_plot$size) # 0.228
...
ggsave("conResults/conSize.pdf", p_size, height = 6.5, width = 13.5)
...
# MAEE
data_plot <- final_con_MAEE[, -(17:22)]
data_plot <- data_plot[, -15]
data_plot <- data_plot[-(31:36), ]
data_plot$model <- c(rep(rep(c("Model 1. NT-CTE", "Model 2. LT-CTE", "Model 3. CT-CTE", "Model 4. LT-TI", "Model 5. CT-TI"), each=6)))
data_plot$scenario <- seq(1, 30, 1)

data_plot <- melt(data_plot, id.vars = c("b", "alpha0", "alpha1", "alpha2", "N", "I", "K", "t", "phi", "model", "scenario", "a_power"),
                  measure.vars = c("MB", "ROB", "KC", "MD"),
                  variable.name = "var", value.name = "power")
data_plot$pd <- data_plot$power - data_plot$a_power
min(data_plot$pd) # -0.01924228
max(data_plot$pd) # 0.04383574
...
P_MAEE <- ggplot(data=data_plot, aes(scenario, pd, colour = factor(var), shape = factor(var))) +
  ...
  facet_grid(. ~ model, scale="free_x", space="free_x") +
  theme(legend.position = "bottom", legend.box = "vertical", legend.margin = margin()) +
  scale_y_continuous(limits = c(-0.025, 0.125), breaks = seq(-0.025, 0.125, 0.025), labels = comma) +
  scale_x_continuous(breaks = seq(1, 30, 2)) +
  guides(colour = guide_legend(order = 1, override.aes = list(shape = c(8, 18, 16, 15)))) +
  guides(shape = "none")

# ind
data_plot <- final_con_ind[, -(17:22)]
data_plot <- data_plot[, -15]
data_plot <- data_plot[-(31:36), ]
data_plot$model <- c(rep(rep(c("Model 1. NT-CTE", "Model 2. LT-CTE", "Model 3. CT-CTE", "Model 4. LT-TI", "Model 5. CT-TI"), each=6)))
data_plot$scenario <- seq(1, 30, 1)
...
ggsave("conResults/conPower.pdf", p_power, height = 6.5, width = 13.5)
