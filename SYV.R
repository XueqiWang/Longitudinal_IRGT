...
conPOWER <- function(N,I,K,t,phi,b,alpha){
  ...
  t_power <- pt(qt(0.05/2, df=I-2)+abs(b)*sqrt(I/sigma2), df=I-2)
  return(data.frame(alpha0=alpha[1], alpha1=alpha[2], alpha2=alpha[3], t_test=t_power))
}

# Function of power contours for longitudinal IRGT trials, with with continuous outcomes under the canonical
# identity link function, assuming no interaction between time and intervention
conPLOT <- function(N,I,K,t,phi,b,a0,a1t0_range,a2_range,pc=0.5,randomization=4){
  con_plot<-NULL
  syv<-NULL
  for (a1t0 in seq(a1t0_range[1], a1t0_range[2], 0.01)){
  ...
  fig<-ggplot()  +
    ...
    xlab(expression(alpha[1]/alpha[0])) +
    ylab(expression(alpha[2])) +
    ylim(c(a2_range[1], a2_range[2])) +
    scale_x_continuous(breaks=seq(a1t0_range[1], a1t0_range[2], 0.1)) +
    stat_contour(data = syv, aes(x = alpha1/alpha0, y = alpha2, z = t_test, colour = ..level..), 
...
ss <- conSIMULATE(8, 3, 1, 0.3, c(0.04, 0.03, 0.8)) # N = 400, I = 225, K = 8, power = 0.8536384
ss
conPOWER(400, 225, 8, 3, 1, 0.3, c(0.04, 0.03, 0.8))

### power contours
# Effect size of 0.3 standard deviation (SD) and N = 400, I = 225, K = 8
syv_1_1 <- conPLOT(400, 225, 8, 3, 1, 0.3, 0.03, c(0, 1), c(0.7, 0.9))
syv_1_2 <- conPLOT(400, 225, 8, 3, 1, 0.3, 0.04, c(0, 1), c(0.7, 0.9))
syv_1_3 <- conPLOT(400, 225, 8, 3, 1, 0.3, 0.05, c(0, 1), c(0.7, 0.9))
Figure_6 <- plot_grid(syv_1_1, syv_1_2, syv_1_3, ncol=3)
ggsave("Figure_syv.pdf", height = 5, width = 15)

conPOWER(400, 225, 8, 3, 1, 0.3, c(0.04, 0.04, 0.9)) # 0.8220284
...
### results
b3 <- c(0.3, 0.1)
ss <- conSIMULATE(8, 3, 1, b3, c(0.04, 0.03, 0.8)) # N = 144, I = 81, K = 8, power = 0.8761613
ss
...
### results
b2j <- c(0.5, 0.3, 0.1)
ss <- conSIMULATE(8, 3, 1, b2j, c(0.04, 0.03, 0.8)) # N = 128, I = 72, K = 8, power = 0.8720563
ss
