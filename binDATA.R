...
binRES<-function(model,row,case,ind){
...
# Collect data
source("binScenarios_Analytical.R")
aPower<-read.xlsx("binResults/pred_power_bin.xlsx", colNames=TRUE)
...
for (ind in 1:2){
  ...
  for (model in c(1:5)){
    for (row in 1:6){
      # Size
      sim_size<-rbind(sim_size,binRES(model=model,row=row,case=1,ind=ind))
      
      # Power
      sim_power<-rbind(sim_power,binRES(model=model,row=row,case=2,ind=ind))
  ...
  final_bin<-cbind(aPower, sim_power, sim_size)
  write.xlsx(final_bin, file = paste("binResults/final_bin_",namei,".xlsx", sep=""), row.names = FALSE)
}
