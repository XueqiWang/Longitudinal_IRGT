...
binSIMULATE<-function(N,I,K,t,beta,alpha,model){
  ...
  for(i in 1:nrep){
    ...
    outfit <- try(GEE.var.kc(y~-1+X,id=id,data=mydata,family=binomial,corstr="independence"),silent=TRUE)
    if (class(outfit)[1]=="try-error" | is.null(outfit)==1){
      nerror <- nerror+1
      results[i,10] <- 1
    } else {results[i,5] <- sqrt(outfit$cov.beta[p])}
    
    outfit <- try(GEE.var.md(y~-1+X,id=id,data=mydata,family=binomial,corstr="independence"),silent=TRUE)
    if (class(outfit)[1]=="try-error" | is.null(outfit)==1){
      nerror <- nerror+1
      results[i,10] <- 1
...
# (1)
setwd('binResults/binRData/2model/1row')
binSIMULATE(220, 121, 10, 3, beta, c(0.03, 0.015, 0.2), 2) # N=220, I=121

# (2)
setwd('../2row')
binSIMULATE(300, 165, 10, 3, beta, c(0.1, 0.05, 0.2), 2) # N=300, I=165

# (3)
setwd('../3row')
binSIMULATE(240, 132, 10, 3, beta, c(0.01, 0.005, 0.4), 2) # N=240, I=132

# (4)
setwd('../4row')
binSIMULATE(200, 110, 10, 4, beta, c(0.03, 0.015, 0.2), 2) # N=200, I=110

# (5)
setwd('../5row')
binSIMULATE(280, 154, 10, 4, beta, c(0.1, 0.05, 0.2), 2) # N=280, I=154

# (6)
setwd('../6row')
binSIMULATE(240, 132, 10, 4, beta, c(0.01, 0.005, 0.4), 2) # N=240, I=132

# Size

beta <- c(log(0.7), log(1.2), 0)
